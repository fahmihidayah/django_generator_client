...
def read_file(file_name, parent_path: str = None):
    with open(os.path.join(parent_path if parent_path else os.path.dirname(os.path.abspath(__file__)), file_name), "r") as f:
        template_string = ''
...
class GeneratorClient:
    ...
    def write_target_model(self, target_model: TargetModel):
        for file_item in FILE_LIST:
            write_string(target_model.directory, '{}.py'.format(file_item), target_model.result[file_item])

        target_model.make_template_dir()

        for file_item in HTML_FILE_LIST:
            write_string(target_model.get_full_template_dir(), '{}_{}.html'.format(target_model.app_dir, file_item), target_model.result['templates'][file_item])
    ...
    def generate_model(self):
        ...
        for target_model in self.target_models:
            response = requests.post(self.url, data={'username': self.username, 'password': self.password, 'models': target_model.model_str})
            log(response.text)
